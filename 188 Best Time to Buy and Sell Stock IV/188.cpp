...
// only need one vector
// for each of the k transactions, we determine when to do the next optimal transaction - the inner for loop
// currentTransaction[i] - max profit from the first i days after each transaction
class Solution5 {
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        vector<int> currentTransaction(n + 1, 0);  // balance we have after completing this transaction (each loop is a transaction)
        for (int i = 0; i < k; ++i)  // k loops
            // balance is the amount of money we would have IF we are in middle of a transaction
            // note where j starts from and the initial value of lastTransaction, because we wouldn't get more profits from 0 to 2i-1 days
            //for (int j = 0, balance = INT_MIN, lastTransaction = 0; j < n; ++j)  // also correct
            for (int j = 2 * i, balance = INT_MIN, lastTransaction = currentTransaction[2 * i]; j < n; ++j)
            {
                // lastTransaction & currentTransaction --- after selling (since last completed transaction)
                // - price                              --- buy
                // balance                              --- after buying, before selling
                // + price                              --- sell

                // whether to start a new transaction at prices[j]
                // lastTransaction is the max profit after i transactions made within first j days (0 through j-1)
                balance = max(balance, lastTransaction - prices[j]);  // for this transaction, find the optimal day to buy
                lastTransaction = currentTransaction[j + 1];
                // whether to finish a transaction at prices[j]
                currentTransaction[j + 1] = max(currentTransaction[j], balance + prices[j]);  // for this transaction, find the optimal day to sell
            }

        return currentTransaction[n];
    }
...
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        if (k >= n / 2)
            return maxProfitInf(prices);
        vector<int> lastTransaction(n + 1, 0), currentTransaction(n + 1, 0);
        for (int i = 0; i < k; ++i)  // k loops
        {
            // the amount of money we WOULD have if we are in the middle of a transaction
            int balance = INT_MIN;
            for (int j = 0; j < n; ++j)
            {
                // lastTransaction & currentTransaction --- after selling (since last completed transaction)
                ...
                balance = max(balance, lastTransaction[j] - prices[j]);
                // whether to finish a transaction
                currentTransaction[j + 1] = max(currentTransaction[j], balance + prices[j]);
            }
...
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        // at most n/2 meaningful (after combining) transactions
        // so it's the same as doing as many as one wants
        if (k >= n / 2)
            return maxProfitInf(prices);

        // balance - the balance during j(th) transaction from day 0 to i with j transactions
        // profit  - the profit after exactly j transactions from day 0 to i with j transactions
        vector<int> balance(k + 1, INT_MIN), profit(k + 1, 0);

        for (int i = 0; i < n; ++i)
            for (int j = 1; j <= i + 1 && j <= k; ++j)
            {
                //                                 the latter term is of day i-1, means not doing transaction on ith day
...
// we can either make one transaction at [v1, p2] (which is always better than making only one of [v1, p1] and [v2, p2])
// or make two at both [v1, p1] and [v2, p2]

// The trick is to treat [v1, p2] as the first transaction, and [v2, p1] as the second
// Then we can guarantee the right max profits in both situations, p2 - v1 for one transaction and p1 - v1 + p2 - v2 for two
// if p1 < v2 (no overlap), we make one transaction
// if p1 > v2 (have overlop), if we have 2 quota, we can make 2 transaction (if both of them ranked top k)
//                            if we have only 1 quota left, we make 1 transaction

// since we pop (v1, p1) and push (v1, p2) onto stack
// if there's a third pair, which v2 <= v3 and p2 <= p3
// then v1 <= v2 <= v3, so we can combine the three of them

// the way we push and pop the stack
// the value of the valleys will be increasing (if smaller than previous, previous will be popped)
// the value of the peaks will be decreasing (if higher than previous, they will be combined)

// keeps pushing the new profit into the priority queue (i.e., the max heap)
// its time complexity should be O(nlog(n) + klog(n)) given that each insertion of a heap costs O(log(n))
class Solution2 {
public:
    int maxProfit(int k, vector<int>& prices) {
        int n = prices.size(), ret = 0, v, p = 0;
        ...
        while (p < n)
        {
            ...
            for (p = v + 1; p < n && prices[p] >= prices[p - 1]; ++p);  // if v == n - 1 then p == n, we are actually saving the real peak + 1
...
public:
    int maxProfit(int k, vector<int>& prices) {
        ...
        while (p + 1 < n)
        {
            ...
            for (p = v + 1; p < n - 1 && prices[p] <= prices[p + 1]; ++p);
            if (p == n)  /////////////////////////////////////////////// different here, so save the peak not peak+1
                break;
            // save profit of 1 transaction at last v/p pair, if current v is lower than last v
            while (!vp_pairs.empty() && prices[v] < prices[vp_pairs.top().first])
            {
...
// a better way would be to push the new profit into a vector and then heapify the vector
// which will reduce the total time complexity to O(n + k*log(n)).
class Solution3 {
public:
    int maxProfit(int k, vector<int>& prices) {
        int ret = 0;
        ...
        // Heapify the vector profits to get the max heap and the time complexity should be O(n).
        make_heap(profits.begin(), profits.end());

        // Sum up first k highest profits.
        for (int i = 0; i < k && !profits.empty(); ++i)
        {
            // Move the max profit to the end of the vector.
...
// could use nth_element() and accumulate(), rather than make_heap() and pop_heap()
// but seems to be extremely slow

//int m = profits.size();
//if (k > m)
//    k = m;
//for (int i = 0; i < k; ++i)
//    nth_element(profits.begin(), profits.end() - i - 1, profits.end() - i);
