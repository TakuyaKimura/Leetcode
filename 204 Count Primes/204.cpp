...
// change from vector to array improve 324ms->76ms
class Solution {
public:
    int countPrimes(int n) {
        //vector<bool> prime_map = vector<bool>(n, true);
        bool *prime_map = new bool[n];
        fill_n(prime_map, n, true);
        int max = sqrt(n), multiple, factor;

        for (int p = 2; p <= max; ++p)
            if (prime_map[p])  // only need to look at multiples of prime numbers
                for (factor = p; (multiple = p * factor) < n; ++factor)  // only need to look at factors that are larger than p
...
public:
    int countPrimes(int n) {
        ...
        // don't need to consider all the even numbers
        int count = 1, i, j, k;  // 2 is prime

        double upper = sqrt(n);

        bool *striked = new bool[n];
        for (i = 3; i < upper; i += 2)
            if (!striked[i])
                // only need to strike out i*i, i(i+2), i(i+4), ...
                // because i(i+1), i(i+3), ... are even numbers
                for (j = i * i, k = 2 * i; j < n; j += k)
                    striked[j] = true;

        // ignore all the even numbers
        for (i = 3; i < n; i += 2)
            if (!striked[i])
                ++count;
        delete[] striked;
